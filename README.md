...
- **Comments are stripped.** SQL comments (`--` and `/* … */`) are removed before fingerprinting, so query-tagging comments (trace IDs, request IDs, sqlcommenter-style annotations) never appear in `db.query.text`, `db.query.fingerprint`, or `db.query.hash`, and cannot fragment query grouping. Unterminated quote or comment constructs are blanked to the end of the input (fail-closed), so malformed raw SQL degrades to a shorter fingerprint rather than leaking trailing content.
...
- **Instrumentation never breaks a query.** Every analysis and OTel call is wrapped; if instrumentation itself throws internally, the query still executes un-instrumented and a capped `diag.warn` (OpenTelemetry diagnostics, never `console`; at most 10 warnings per failure class per process) reports it. Query errors are always rethrown to the caller unchanged — this library never wraps, swallows, or alters them.
- **Driver error messages are a separate channel.** On query/transaction failure, the span status `message` is set by default to `error.message` from the database driver, and (when `recordExceptions: true`, the default) `span.recordException(error)` records that same driver error (message + stack) as a span event. This is the driver's own error, not something this library extracts — but driver error text (e.g. Postgres constraint/type-violation messages) can echo a submitted value. If you have strict data-governance requirements, set `recordExceptions: false` to drop the exception event, and `recordErrorMessages: false` to suppress the span status message, leaving only `ERROR` status and `error.type`.
...
- **Quoted table names in raw SQL are extracted only when they are simple.** In hand-written `sql`/`sql.raw` fragments, a quoted identifier holding one plain name (`FROM "orders"`, `` FROM `orders` ``, `FROM [orders]`, `FROM "public"."orders"`) is extracted normally; a quoted name containing spaces or punctuation (`FROM "my table"`) is skipped rather than misread — surrounding SQL keywords are never reported as table names. Builder queries are unaffected (table names come from the AST).
- **Savepoint statements are not traced.** `SAVEPOINT` / `ROLLBACK TO SAVEPOINT` / `RELEASE SAVEPOINT` statements issued through Kysely's savepoint API are forwarded directly to the inner driver and emit no spans; the surrounding `TRANSACTION` span (and every query span inside it) is unaffected.
