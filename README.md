...
- **Quoted table names in raw SQL are extracted only when they are simple.** In hand-written `sql`/`sql.raw` fragments, a quoted identifier holding one plain name (`FROM "orders"`, `` FROM `orders` ``, `FROM [orders]`, `FROM "public"."orders"`) is extracted normally; a quoted name containing spaces or punctuation (`FROM "my table"`) is skipped — that query reports no tables rather than a wrong one. Builder queries are unaffected (table names come from the AST).
